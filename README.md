...
- J21 APPLY first calls `LoadThisPartFully()` on each unique BoM-visible target that needs it and re-traverses until newly exposed descendants are loaded. It intentionally does not call assembly-wide `LoadFully()`, so suppressed/reference/keyword-excluded structure remains out of scope. Any required load failure writes part, diagnostic, and run-summary rows to the J21 CSV and aborts every mass update/save without a traceback. After the load gate passes, J21 processes deepest leaves first, then subassemblies, and the active assembly last. For each writable target it temporarily makes that part the work part and drives NX's native `CreateMassPropertiesBuilder` path (`UpdateOnSave` + `UpdateNow` + `Commit`), saves, and reads back `NX_MassPropRollupMass` (kg) / `NX_MassPropRollupArea` (mm^2). J21 never calls a checkout API or writes reserved titles directly; checked-in, read-only, and other-user targets are skipped and reported. `NX_J21_MODE=DRY_RUN` does not load or save, while `NX_J21_MODE=SMOKE` loads and updates only the active work part.
