...
- J21 does NOT compute or write attributes itself: it triggers NX's native Mass Properties update (Measure > Mass Properties with Roll Up + Update On Save, via `PropertiesManager.CreateMassPropertiesBuilder` + `UpdateNow`) on the open assembly, so NX itself writes its standard `NX_MassPropRollupMass` (kg) and `NX_MassPropRollupArea` (mm^2) attributes under `Rolled-Up Mass Properties` on every component. It then saves each BoM-visible 3D master (same filter as `NXOpenBoMExtended.py`/J04) and reads the standard attributes back into the report so the run proves what NX wrote. Because NX stores area in mm^2 (PDM template), J21's report adds an m^2 column and the extended BoM exports `NX_MassPropRollupArea_m2` (m^2), so large-system values stay readable. `WRITE_MODE` defaults to `APPLY`; set `WRITE_MODE = "DRY_RUN"` (or `NX_J21_MODE=DRY_RUN`) to report current values without updating/saving, or `NX_J21_MODE=PROBE` to dump the builder API surface of your NX build. Teamcenter parts must be writable (checked out) for the save to succeed; otherwise the part is reported `SAVE_FAILED`.
