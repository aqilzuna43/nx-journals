...
| 21 | `from_git/journals/21_mass_surface_attribute_updater.py` | Fully loads the active BoM-visible subtree in memory, verifies the load gate, then triggers NX's native mass-properties update separately on every unique leaf part and subassembly in bottom-up order. Load failures produce a diagnostic CSV and abort all mass updates cleanly. It never checks out or directly writes reserved attributes; non-writable Teamcenter targets are skipped and reported. DRY_RUN / SMOKE / PROBE modes. |
...
| 36 | `from_git/journals/36_session_tree_pdf_step_export.py` | Session-tree packager: no operator CSV. Defaults to `DRY_RUN`; fully loads every BoM-visible level of the open assembly (Journal 21 load gate with descendant re-discovery), resolves each level's drawings from the session then from Teamcenter `dwg1..dwg9`, and exports one PDF per drawing specification plus one monolithic AP214 STEP per level. Skips existing outputs, never checks out or saves, restores the original display/work parts, and writes a J07-compatible `EXPORT_RESULT` report with `LEVEL` / `DEEPEST_LEVEL` / `OCCURRENCE_COUNT` columns |
| 37 | `from_git/journals/37_diagnose_display_performance.py` | Read-only model-view triage for "a simple view change takes 10 minutes". Inventories the whole assembly: visible-object census of the active view (`View.AskVisibleObjects` by NX type), load truth (`PartLoadState`, `HasAnyMinimallyLoadedChildren`), per-occurrence `RepresentationMode` / `ReferenceSet` / `EntirePartRefsetName` / suppression / layer / blanking, per-prototype bodies-faces-edges-facets-vertices and per-body `Density`, datum/curve/point clutter, True Shading / studio / point-cloud / decal objects, a 256-layer census with states, and the display preferences that make a view update reload or re-tessellate (`LoadComponentOnFacetedViewUpdate`, `RenderSolidsUsingStoredFacets`, shading tolerances, `WorkPartDisplayAsEntirePart`). `NX_J37_MODE=TIMED` additionally measures `Regenerate` / `Rotate` / `Fit` in seconds and restores the exact view state. Outputs a ranked suspect list where every line cites a measured value and a fact ID; never loads, changes, updates, saves, checks out, or opens Teamcenter data |
| 38 | `from_git/journals/38_attribute_variant_probe.py` | Read-only native-file attribute variant probe for the customer-TC duplicate-attribute question: does the pushed .prt already carry BOTH the stored title (`Commodity_Code`) and the template alias (`Commodity Code`)? Censuses all 16 WAE canonical fields on the work part plus every unique prototype, flags `BOTH_VARIANTS` / `ALIAS_ONLY` with IDENTICAL vs CONFLICTING value relations, discovers unknown variants via normalized-title collisions, and reports per-attribute `PdmBased` / `OwnedBySystem` flags that identify Teamcenter round-trip mirrors. `NX_FILE_CLEAN` verdicts shift the root cause to the customer's TC conversion mapping; see `docs/J38_ATTRIBUTE_VARIANT_PROBE.md`. Never writes, saves, checks out, or opens Teamcenter data |
...
| J36 | `NX_BULK_EXPORT\<timestamp>\PDF\<number>_REV<rev>.<WAE_VERSION>[_DWG<n>].pdf` and `STEP\<number>_REV<rev>.<WAE_VERSION>.stp`, plus `REPORTS\EXPORT_RESULT_<timestamp>.csv` and `LOGS\EXPORT_LOG_<timestamp>.txt` |
| J37 | `NX_DISPLAY_PERF\<timestamp>\REPORTS\J37_{TARGETS,OCCURRENCES,LAYERS,VISIBLE,PREFS,SUSPECTS,TIMING,DISCOVER}_<timestamp>.csv`, `J37_EVIDENCE_<timestamp>.json` (which also embeds `totals` and the visible-object rows), and `LOGS\J37_LOG_<timestamp>.txt` |
...
- J36 packages the assembly that is open right now and needs no input CSV. It traverses every BoM-visible occurrence with the J04/J21 filter, fully loads unloaded targets with `LoadThisPartFully()` and re-traverses until the scope is stable, and reports load failures per target while sibling branches continue. PDFs reuse the J07 DataPack rendering path (native `DRAFT_<rev>.<WAE_VERSION>` watermark, one temporary `EXPORTED: <date> <time> MYT` note per sheet that is undone to a private undo mark, searchable text) so a J36 PDF and a J07 PDF of the same revision match. STEP is the J10/J07-proven `DisplayPart` + `EntirePart` + `LayerMask 1-256` AP214 combination, one monolithic file per level, exported only for targets that have a drawing unless `NX_J36_STEP_SCOPE=ALL`. Existing outputs are reported as `SKIPPED_EXISTS` and never overwritten. `EXPORT_RESULT_*.csv` keeps the J07 column names, so `from_git/utils/single_drawing_scope.py` can still generate a J25 scope from a J36 run. Switches: `NX_J36_MODE` (`DRY_RUN` default, `APPLY` to export), `NX_J36_SCOPE` (`BOM` default, `ALL` for the raw tree), `NX_J36_LOAD_MODE` (`LOAD` default, `REPORT_ONLY`), `NX_J36_STEP_SCOPE` (`DRAWING_ONLY` default, `ALL`).
- J37 answers "why is this model view so slow" with measurements instead of guesses. Run it on the already-fully-loaded assembly: `NX_J37_MODE=PROBE` (default) never moves the view and writes the inventory, the ranked suspects, and the evidence ledger; `NX_J37_MODE=TIMED` additionally times `View.Regenerate`, `View.UpdateDisplay`, `NX_J37_ROTATIONS` x `View.Rotate` pairs, and `View.Fit`, then restores the exact view `Matrix` / `Origin` / `Scale`. Symptoms it attributes include partial or missing loads (`PartLoadState`, `HasAnyMinimallyLoadedChildren`), `RepresentationMode=Exact` and `ReferenceSet == EntirePartRefsetName` occurrences, high face/facet counts weighted by occurrence count, zero or outlier body `Density`, visible-layer spread and hidden layers still holding objects, blanked-object bulk, True Shading / studio / point-cloud objects, and the preference switches that make each view update load components or re-tessellate. It is strictly read-only: no load, no visibility/suppression/reference-set/layer/arrangement change, no update, no save, no checkout, and no `@DB` access. Every probe that the NX build does not expose is reported `UNAVAILABLE` with the reason and is never converted into `0`, `False`, or a finding. Use `Assembly/Diagnostic/NX_Assembly_Load_Diagnostic.vb` as a companion when the cause may be a missing or unloaded component: it names the exact occurrence, while J37 measures what the view actually pays for. Use J28 when the occurrence structure itself is in question. Switches: `NX_J37_MODE` (`PROBE` default, `TIMED` to measure), `NX_J37_SCOPE` (`ALL` default, `BOM`), `NX_J37_VISIBLE_SCAN`, `NX_J37_ROTATIONS`, `NX_J37_MAX_BODIES`, `NX_J37_DISCOVER` (`YES` dumps the preference/component member names this NX build actually exposes, so probe paths can be corrected from evidence). V2 canonicalizes `PartLoadState` (NX 2506 returned `1`/`2` instead of enum names, which made v1 flag every part as not fully loaded), reports unnamed prototypes with their component path, and treats a non-enumerable NX manager (for example `Annotations.AnnotationManager`) as `UNAVAILABLE` rather than `ERROR`.
...
- J25 reduces an exact 3D Item/Revision to one explicitly retained `dwg<n>` specification. It defaults to `DRY_RUN`; apply requires approval, an exact live extra-index list, and backup of every removed payload. NXOpen has no relation-only detach in this API boundary, so J25 removes each extra empty drawing dataset with `DeleteExistingAttachedFiles(..., False)` and then verifies that only the retained drawing remains. Scope rows can be generated from any J07 `EXPORT_RESULT_*.csv` with `from_git/utils/single_drawing_scope.py` (pure Python, no NX); the scope for the 16 offenders found in the 2026-08-15 run is committed at `from_git/templates/NX_TC_SINGLE_DRAWING_SCOPE.csv`.
